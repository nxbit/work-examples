...
# Wait until the Export as CSV Button Appears #
Write-Host "Waiting for the Export as Plain Text Button"
do{
Start-Sleep -Seconds 1
}while($chrome.FindElementsByid('exportFormatGrids_csvIServer').Count -eq 0)
...
Write-Host "Select Export as CSV"
# Click the Plain Text Button #
$chrome.FindElementByid('exportFormatGrids_csvIServer').click()
...
$DldFile = Get-ChildItem $saveFolderPath -Recurse | Where-Object {$_.Mode -notmatch "d"} | Where-Object {$_.LastWriteTime -gt $currentTime} | Where-Object {$_.Extension -eq ".csv"}
...
$dlFile = Get-ChildItem $saveFolderPath -filter *.csv | sort LastWriteTime | select -Last 1

Rename-Item -Path $dlFile.FullName -NewName "$FileName.csv"
