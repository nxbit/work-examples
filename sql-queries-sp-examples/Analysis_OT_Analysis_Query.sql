...
,cast('' as nvarchar(25)) workLocation
,sg.CODE shiftCode
...
update #otTempBase
set workLocation = l.CODE
from MiningSwap.dbo.VID_WAH l with(nolock)
where #otTempBase.EMP_SK = l.EMP_SK and #otTempBase.NOM_DATE between l.START_NOM_DATE and l.STOP_NOM_DATE


select 
MGMTAREANAME, empName, ID, format(stdDate,'MM/dd/yyyy') stdDate, case when OVERTIME is null then 0 else OVERTIME end [OVERTIME]
from (
select 
ot.MGMTAREANAME
,w.FIRSTNAME+' '+w.LASTNAME empName
,e.ID
,DATEADD(day, ot.NOM_DATE, '18991230') stdDate
,ot.shiftCode
,ot.DurHrs - case when ot.lunchDur is null then 0 else ot.lunchDur end DurHrs
...
where ot.workLocation like 'Remote'
)t 
PIVOT
(
sum(DurHrs)
...
left join MiningSwap.dbo.VID_EMP e with(nolock) on ot.EMP_SK = e.EMP_SK
left join MiningSwap.dbo.PROD_WORKERS w with(nolock) on e.ID = w.NETIQWORKERID
where ot.workLocation like 'In Center'
)t 
PIVOT
(
sum(DurHrs)
...
select 
b.MGMTAREANAME siteName
,b.FiscalMth payWeek
,b.workLocation WorkLocation
,max(e.empCount) WorkLocationCount
,count(distinct EMP_SK) EmployeeCount
,shiftCode ShiftType
,case when sum(DurHrs) is null then 0 else sum(DurHrs) end - 
case when sum(lunchDur) is null then 0 else sum(lunchDur) end DUR

from #otTempBase b
/* Grabbing EMP Counts by Worklocation by WEThrus */
left join (
	select 
		MGMTAREANAME
		,FiscalMth
		,workLocation
		,count(distinct EMP_SK) empCount
	from #otTempBase
	group by MGMTAREANAME, FiscalMth, workLocation) e 
on b.MGMTAREANAME = e.MGMTAREANAME 
and b.FiscalMth = e.FiscalMth
and b.workLocation = e.workLocation
