...
;
  ...
  ;;    
	...
	select    
		c.AgentUCID AgUCID   
		,CASE
			WHEN DATEPART(dd,c.AgentRealStartTime) < 29 THEN (DATEPART(yyyy,c.AgentRealStartTime)*100)+DATEPART(QQ,c.AgentRealStartTime)
			ELSE (DATEPART(yyyy,DATEADD(d,14,c.AgentRealStartTime))*100)+DATEPART(QQ,DATEADD(d,14,c.AgentRealStartTime))
		END Qrtr
	...
	SELECT   
		CASE
			WHEN DATEPART(dd,ldsCall.LeadRealStartTime) < 29 THEN (DATEPART(yyyy,ldsCall.LeadRealStartTime)*100)+DATEPART(QQ,ldsCall.LeadRealStartTime)
			ELSE (DATEPART(yyyy,DATEADD(d,14,ldsCall.LeadRealStartTime))*100)+DATEPART(QQ,DATEADD(d,14,ldsCall.LeadRealStartTime))
		END Qrtr
