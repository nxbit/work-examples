/*VM0PWOWMMSD0004.corp.chartercom.com*/
SET NOCOUNT ON;
;
/*
Modeled based on the original lead repeats query;
;
...
with params as (
	SELECT
	cast(DateAdd(week,-1,DATEADD(dd, 7-(DATEPART(dw, @chkDate)), @chkDate)) as date) as eDate
	,cast(DateAdd(day,-27,DateAdd(week,-1,DATEADD(dd, 7-(DATEPART(dw, @chkDate)), @chkDate))) as date) as sDate
)
,ldSK as (
...
SELECT 
	...
	,DateAdd(d,-1,getutcdate()) prevDay
...
--LEFT OUTER JOIN GVPOperations.VID.ATT_WAH w with(nolock)
	...
	--and DateDiff(d,'12/30/1899',DateAdd(d,-1,getutcdate())) between w.START_NOM_DATE and case when w.STOP_NOM_DATE is null then DateDiff(d,'12/30/1899',DateAdd(d,1,getutcdate())) else w.STOP_NOM_DATE end
