declare	@fiscalNom int = 202307;
	...
	*/
	...
	delete from #R_Roster where id in (select se.PSID from GVPOperations.VID.R_Sup_Exclusions se with(nolock)
		where case 
		when se.StartDate between @sDate and @eDate then 1
		when se.EndDate between @sDate and @eDate then 0
		when se.StartDate <= @sDate and se.EndDate >= @eDate then 1
		else 0
		end = 1) and posLevel <> 5;
	...
	if abs(datediff(day,@sDate,getutcdate())) >= 14 begin
		delete from #R_Roster where (calls < 100 or staffedTime < 80.00) and posLevel <> 5;
		/* Leads have moved to using 250 Calls as a Minimum as of Fiscal April '23 */
		if @fiscalNom < 202304 begin
			delete from #R_Roster where ((calls < 100 or staffedTime < 80.00) or (calls is null or staffedTime is null)) and posLevel = 4;
		end
		else
		begin
			delete from #R_Roster where ((calls < 250 or staffedTime < 80.00) or (calls is null or staffedTime is null)) and posLevel = 4;
		end;
